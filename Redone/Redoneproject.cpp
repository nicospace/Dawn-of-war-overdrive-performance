#ifndef PCH_H
#define PCH_H

// Windows Version
#define _WIN32_WINNT 0x0600
#define WIN32_LEAN_AND_MEAN   // Reduce Windows headers bloat
#define NOMINMAX              // Prevent Windows min/max macros

// Exclude ImageHlp.h to avoid duplicate struct definitions
#define NO_IMAGEHLP

// Windows API Headers
#include <windows.h>
#include <DbgHelp.h>   // Do NOT include ImageHlp.h
#include <shlwapi.h>
#include <commdlg.h>
#include <shellapi.h>
#include <tchar.h>
#include <psapi.h>
#include <iostream>
// Standard C++ Headers
#include <vector>
#include <string>
#include <cstdint>
#include <thread>
#include <mutex>
#include <cstring>
#include <fstream>

#include <algorithm>
#include <cstdio>
#include <cstdlib>
#include <cstdarg>
#include <unordered_map>
#include <array>

// Custom Project Includes
#include "ZeroPEChecksum.h"

// Required Linker Directives
#pragma comment(lib, "advapi32.lib")
#pragma comment(lib, "user32.lib")
#pragma comment(lib, "shell32.lib")
#pragma comment(lib, "shlwapi.lib")
#pragma comment(lib, "comdlg32.lib")
#pragma comment(lib, "Dbghelp.lib")
#pragma comment(lib, "psapi.lib")

#pragma comment(linker, "/SECTION:.injsec,RWE")

#endif // PCH_H





// Ensure the .injsec section exists by placing a dummy variable in it.
// --------------------
// Force creation of .injsec section
// --------------------
// Use shared read and execute attributes (remove "write" if causing warnings)
#pragma section(".injsec", read, execute, shared)
__declspec(allocate(".injsec")) char dummyInjSec = 0;



// --------------------------------------------------
// Debug Logger
// --------------------------------------------------
class DebugLogger {
private:
    static const int LOG_BUFFER_SIZE = 1024;
    static const char* LOG_FILE_NAME;
    static FILE* logFile;
public:
    enum LogLevel { INFO, WARNING, CRITICAL };
    static void Init(const char* fileName = nullptr);
    static void Log(LogLevel level, const char* fmt, ...);
    static void Cleanup();
};

const char* DebugLogger::LOG_FILE_NAME = nullptr;
FILE* DebugLogger::logFile = nullptr;

void DebugLogger::Init(const char* fileName) {
    static char temp[256];
    if (fileName)
        LOG_FILE_NAME = fileName;
    else {
        SYSTEMTIME st;
        GetSystemTime(&st);
        sprintf_s(temp, "patch_debug_%04d%02d%02d_%02d%02d%02d.log",
            st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
        LOG_FILE_NAME = temp;
    }
    fopen_s(&logFile, LOG_FILE_NAME, "w+");
    if (logFile)
        Log(CRITICAL, "=== Starting debug session ===");
}

void DebugLogger::Log(LogLevel level, const char* fmt, ...) {
    if (!logFile) return;
    va_list args;
    va_start(args, fmt);
    char buffer[LOG_BUFFER_SIZE];
    vsnprintf_s(buffer, LOG_BUFFER_SIZE, _TRUNCATE, fmt, args);
    const char* levelStr = "";
    switch (level) {
    case INFO:     levelStr = "INFO"; break;
    case WARNING:  levelStr = "WARN"; break;
    case CRITICAL: levelStr = "CRIT"; break;
    }
    fprintf(logFile, "[%s] %s\n", levelStr, buffer);
    fflush(logFile);
    va_end(args);
}

void DebugLogger::Cleanup() {
    if (logFile) {
        fclose(logFile);
        logFile = nullptr;
    }
}

//HOOK DLL
// HERE FOR RENDER


// Custom exception class for DLL-related errors
class DllError : public std::exception {
    DWORD errorCode;
    std::string message;

public:
    DllError(DWORD code, const std::string& msg)
        : errorCode(code), message(msg) {
    }

    const char* what() const noexcept override {
        return message.c_str();
    }
};
// RAII wrapper for DLL handles
class DllHandle {
public:
    DllHandle() : handle_(nullptr) {}
    explicit DllHandle(HMODULE handle) : handle_(handle) {}
    ~DllHandle() { if (handle_) FreeLibrary(handle_); }

    DllHandle(const DllHandle&) = delete;
    DllHandle& operator=(const DllHandle&) = delete;

    DllHandle(DllHandle&& other) noexcept : handle_(other.handle_) {
        other.handle_ = nullptr;
    }

    DllHandle& operator=(DllHandle&& other) noexcept {
        if (this != &other) {
            if (handle_) FreeLibrary(handle_);
            handle_ = other.handle_;
            other.handle_ = nullptr;
        }
        return *this;
    }

    operator HMODULE() const { return handle_; }
    HMODULE get() const { return handle_; }

private:
    HMODULE handle_;
};

// Load a DLL with proper error handling and check if already loaded


DllHandle loadDll(const std::wstring& path) {
    HMODULE handle = GetModuleHandleW(path.c_str());
    if (handle) {
        return DllHandle(handle);
    }

    // Check if the DLL file exists at the specified path
    DWORD dwAttrib = GetFileAttributesW(path.c_str());
    if (dwAttrib == INVALID_FILE_ATTRIBUTES || !(dwAttrib & FILE_ATTRIBUTE_DIRECTORY)) {
        // DLL not found, log the absence
        std::string pathStr(path.begin(), path.end()); // Convert wstring to string
        std::cerr << "DLL not found: " << pathStr << std::endl;
        return DllHandle(); // Return a null handle
    }

    // DLL is found, attempt to load it
    handle = LoadLibraryW(path.c_str());
    if (!handle) {
        const DWORD errorCode = GetLastError();
        std::string pathStr(path.begin(), path.end()); // Convert wstring to string
        std::cerr << "Failed to load DLL: " << pathStr << " with error code: " << errorCode << std::endl;
        return DllHandle(); // Return a null handle if loading fails
    }

    std::string pathStr(path.begin(), path.end()); // Convert wstring to string
    std::wcout << L"Successfully loaded DLL: " << path.c_str() << std::endl;
    return DllHandle(handle);
}


// Get a function pointer from a DLL
template<typename FuncType>
FuncType getProcAddress(DllHandle& dll, const std::string& funcName) {
    const FARPROC proc = GetProcAddress(dll.get(), funcName.c_str());
    if (!proc) {
        const DWORD errorCode = GetLastError();
        throw DllError(errorCode, "Failed to find function '" + funcName + "'");
    }
    return reinterpret_cast<FuncType>(proc);
}

// Declare missing handles
HMODULE g_hGDIHooking = nullptr;
HMODULE g_hSDL2 = nullptr;
HMODULE g_hPleasework = nullptr;

// Cleanup function to unload DLLs




int main() {
    try {
        // Define paths as const variables
        const std::wstring pleaseWorkPath = L"C:\\Program Files (x86)\\Steam\\steamapps\\common\\Dawn of War Soulstorm\\Please work now.dll";
        const std::wstring sdl2Path = L"C:\\Program Files (x86)\\Steam\\steamapps\\common\\Dawn of War Soulstorm\\SDL2.dll";
        const std::wstring gdiHookPath = L"C:\\Program Files (x86)\\Steam\\steamapps\\common\\Dawn of War Soulstorm\\GDI HOOKING DLL.dll";

        // Load DLLs with RAII
        DllHandle pleaseWorkDll = loadDll(pleaseWorkPath);
        DllHandle sdl2Dll = loadDll(sdl2Path);
        DllHandle gdiHookDll = loadDll(gdiHookPath);

    
    
        // Set the global handles
        g_hGDIHooking = gdiHookDll.get();
        g_hSDL2 = sdl2Dll.get();
        g_hPleasework = pleaseWorkDll.get();

        // Show success message
        MessageBoxW(nullptr, L"All DLLs loaded and GDI hooking is active!", L"Success", MB_OK);
        std::cout << "All DLLs successfully loaded and GDI hooking is confirmed active." << std::endl;

        return 0;
    }
    catch (const DllError& e) {
        std::wstring errorMessage = std::wstring(e.what(), e.what() + strlen(e.what()));
        MessageBoxW(nullptr, errorMessage.c_str(), L"Error", MB_ICONERROR);
        std::cerr << "Error: " << e.what() << std::endl;
        return 1;
    }
    catch (const std::exception& e) {
        std::wstring errorMessage = std::wstring(e.what(), e.what() + strlen(e.what()));
        MessageBoxW(nullptr, errorMessage.c_str(), L"Error", MB_ICONERROR);
        std::cerr << "Error: " << e.what() << std::endl;
        return 1;
    }
}


// Add cleanup function
void UnhookOurDLLs() {
    if (g_hGDIHooking) FreeLibrary(g_hGDIHooking);
    if (g_hSDL2) FreeLibrary(g_hSDL2);
    if (g_hPleasework) FreeLibrary(g_hPleasework);
}


// Patch memory in the current process
#define PATCH_ADDRESS 0x007D1496

#include <windows.h>
#include <cstdio>
#include <cstdlib>
#include <string>

// Address to patch: Example target for multiplayer check removal
#define PATCH_ADDRESS 0x007D1496

// Patch function that attempts to modify memory at runtime
#include <windows.h>
#include <vector>
#include <iostream>

#define PATCH_SIGNATURE { 0x75, 0x40 } // Original bytes (jnz 0x007D14D8)
#define PATCH_BYTES { 0x90, 0x90 } // NOP NOP replacement

DWORD_PTR FindPatchAddress(BYTE* signature, size_t sigSize) {
    SYSTEM_INFO sysInfo;
    GetSystemInfo(&sysInfo);

    // Set our scanning boundaries.
    DWORD_PTR baseAddress = (DWORD_PTR)sysInfo.lpMinimumApplicationAddress;
    DWORD_PTR maxAddress = (DWORD_PTR)sysInfo.lpMaximumApplicationAddress;
    MEMORY_BASIC_INFORMATION mbi;

    // Allocate a temporary buffer for signature comparison.
    BYTE* buffer = new BYTE[sigSize];

    // Loop through the entire memory range.
    while (baseAddress < maxAddress) {
        // Query the memory region information.
        if (VirtualQuery((LPCVOID)baseAddress, &mbi, sizeof(mbi)) == 0) {
            baseAddress += 0x1000; // Move forward if the query fails.
            continue;
        }

        // Only scan committed memory with read/execute permissions.
        if (mbi.State == MEM_COMMIT &&
            (mbi.Protect & PAGE_EXECUTE_READWRITE || mbi.Protect & PAGE_EXECUTE_READ)) {

            // Compute the upper bound to avoid overrunning the region.
            DWORD_PTR regionEnd = (DWORD_PTR)mbi.BaseAddress + mbi.RegionSize - sigSize;
            for (DWORD_PTR addr = (DWORD_PTR)mbi.BaseAddress; addr < regionEnd; addr++) {
                SIZE_T bytesRead = 0;
                BOOL result = FALSE;

                // Use structured exception handling to catch any access violations.
                __try {
                    result = ReadProcessMemory(GetCurrentProcess(), (LPCVOID)addr, buffer, sigSize, &bytesRead);
                }
                __except (EXCEPTION_EXECUTE_HANDLER) {
                    result = FALSE;
                }

                // If the read succeeded, the number of bytes matches, and the signature matches, return the address.
                if (result && bytesRead == sigSize && memcmp(buffer, signature, sigSize) == 0) {
                    delete[] buffer;
                    return addr;
                }
            }
        }

        // Move to the next memory region.
        baseAddress += mbi.RegionSize;
    }

    delete[] buffer;
    return 0; // Return 0 if the signature wasn't found.
}


void PatchMultiplayerLobby() {
    HANDLE hProcess = GetCurrentProcess();
    if (!hProcess) {
        MessageBoxA(nullptr, "[-] Failed to open process!", "Error", MB_ICONERROR);
        return;
    }

    BYTE signature[] = PATCH_SIGNATURE;
    DWORD_PTR patchAddr = FindPatchAddress(signature, sizeof(signature));
    if (!patchAddr) {
        MessageBoxA(nullptr, "[-] Could not find patch address!", "Error", MB_ICONERROR);
        return;
    }

    MEMORY_BASIC_INFORMATION mbi;
    if (VirtualQuery((LPCVOID)patchAddr, &mbi, sizeof(mbi)) == 0) {
        MessageBoxA(nullptr, "[-] VirtualQuery() failed! Address is invalid.", "Error", MB_ICONERROR);
        return;
    }

    DWORD oldProtect;
    if (VirtualProtect((LPVOID)patchAddr, 2, PAGE_EXECUTE_READWRITE, &oldProtect)) {
        BYTE patchBytes[] = PATCH_BYTES;
        SIZE_T bytesWritten;
        if (WriteProcessMemory(hProcess, (LPVOID)patchAddr, patchBytes, sizeof(patchBytes), &bytesWritten)) {
            VirtualProtect((LPVOID)patchAddr, 2, oldProtect, &oldProtect);
            MessageBoxA(nullptr, "[+] Successfully patched multiplayer lobby!", "Success", MB_OK);
            return;
        }
    }

    MessageBoxA(nullptr, "[-] Failed to patch memory!", "Error", MB_ICONERROR);
}

// Global pointers for the original functions
//lockstep related

// Finds the first occurrence of a pattern in a block of memory.
// 'pattern' is an array of bytes; 'mask' is a string of the same length 
// where 'x' means an exact match.
BYTE* FindPattern(BYTE* base, DWORD size, const BYTE* pattern, const char* mask) {
    DWORD patternLength = (DWORD)strlen(mask);
    for (DWORD i = 0; i <= size - patternLength; i++) {
        bool found = true;
        for (DWORD j = 0; j < patternLength; j++) {
            if (mask[j] == 'x' && pattern[j] != *(base + i + j)) {
                found = false;
                break;
            }
        }
        if (found)
            return base + i;
    }
    return nullptr;
}

// Structure to hold patch data.
struct Patch {
    DWORD offset;
    std::vector<BYTE> bytes;

    Patch(DWORD _offset = 0) : offset(_offset) {}
};





//lockstep related
// above should be sleeping and wait for functions
// -------------------------------------------------------------------------------------
// Memory Pool Sizes (Preserve Network Allocations)
// -------------------------------------------------------------------------------------


// ---------------------------------------------------------------------------
// Memory Pool Configuration
// ---------------------------------------------------------------------------
constexpr size_t RESERVED_SYSTEM_MEMORY = 512ull * 1024 * 1024;   // 512MB for game-critical allocations
constexpr size_t PRIVATE_MEMORY_SIZE = 1ull * 1024 * 1024 * 1024; // 1GB reserve
constexpr size_t TEXTURE_MEMORY_SIZE = 1ull * 1024 * 1024 * 1024; // 1GB textures
constexpr size_t TOTAL_POOL_SIZE = PRIVATE_MEMORY_SIZE + TEXTURE_MEMORY_SIZE;

// ---------------------------------------------------------------------------
// Memory Block & Pool Structures
// ---------------------------------------------------------------------------
struct MemoryBlock {
    size_t size;        // allocation size (16-byte aligned)
    bool   used;        // true if currently allocated
    void* address;     // pointer into our pool
    MemoryBlock* next;  // link for tracking (used for free-list and debugging)
};

struct MemoryPool {
    char* pool;                         // base address of our reserved pool
    std::atomic<size_t> offset;         // current allocation offset (lock-free add)
    std::atomic<MemoryBlock*> head;     // linked list of all allocated blocks (for debugging/tracking)
    std::atomic<MemoryBlock*> freeList; // lock-free free-list for reusing freed blocks

    MemoryPool() : pool(nullptr), offset(0), head(nullptr), freeList(nullptr) {}
};

static MemoryPool gameMemoryPool;

// Fixed: Defined the missing variables
static std::once_flag initPoolFlag;
static std::mutex headMutex;  // Protect head-list scanning

// If your patching code uses a structure named Patch, ensure its members are properly initialized:

// ---------------------------------------------------------------------------
// Initialization: Reserve the pool memory once (thread-safe).
// ---------------------------------------------------------------------------
void InitMemoryPool() {
    std::call_once(initPoolFlag, []() {
        char* p = static_cast<char*>(VirtualAlloc(nullptr, TOTAL_POOL_SIZE, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE));
        if (!p) {
            MessageBoxA(nullptr, "Failed to allocate game memory pool!", "Error", MB_OK | MB_ICONERROR);
            exit(1);
        }
        gameMemoryPool.pool = p;
        gameMemoryPool.offset.store(0, std::memory_order_relaxed);
        gameMemoryPool.head.store(nullptr, std::memory_order_relaxed);
        gameMemoryPool.freeList.store(nullptr, std::memory_order_relaxed);
        });
}
void CleanupMemoryPool() {
    if (gameMemoryPool.pool) {
        // First, clean up all memory blocks
        {
            std::lock_guard<std::mutex> lock(headMutex);
            MemoryBlock* current = gameMemoryPool.head.load(std::memory_order_acquire);
            while (current) {
                MemoryBlock* next = current->next;
                delete current;
                current = next;
            }
        }

        // Free the pool itself
        VirtualFree(gameMemoryPool.pool, 0, MEM_RELEASE);
        gameMemoryPool.pool = nullptr;
        gameMemoryPool.offset.store(0, std::memory_order_relaxed);
        gameMemoryPool.head.store(nullptr, std::memory_order_relaxed);
        gameMemoryPool.freeList.store(nullptr, std::memory_order_relaxed);
    }
}







// ---------------------------------------------------------------------------
// Helper: Determine if pointer 'p' belongs to our custom pool.
// ---------------------------------------------------------------------------
bool IsInCustomPool(void* p) {
    return gameMemoryPool.pool && (p >= gameMemoryPool.pool && p < gameMemoryPool.pool + TOTAL_POOL_SIZE);
}

// ---------------------------------------------------------------------------
// Lock-Free Free-List Helpers
// ---------------------------------------------------------------------------
inline MemoryBlock* PopFreeBlock() {
    MemoryBlock* block = gameMemoryPool.freeList.load(std::memory_order_acquire);
    while (block) {
        MemoryBlock* next = block->next;
        if (gameMemoryPool.freeList.compare_exchange_weak(block, next, std::memory_order_acq_rel, std::memory_order_acquire))
            return block;
    }
    return nullptr;
}

inline void PushFreeBlock(MemoryBlock* block) {
    MemoryBlock* oldFree = gameMemoryPool.freeList.load(std::memory_order_acquire);
    do {
        block->next = oldFree;
    } while (!gameMemoryPool.freeList.compare_exchange_weak(oldFree, block, std::memory_order_acq_rel, std::memory_order_acquire));
}

// ---------------------------------------------------------------------------
// Custom Malloc (Lock-Free Allocation from Pool)
// ---------------------------------------------------------------------------
extern "C" __declspec(dllexport) void* CustomMalloc(size_t size) {
    if (size == 0)
        return nullptr;

    // Delegate very small or very large allocations to system malloc.
    if (size < 1024 || size >(TOTAL_POOL_SIZE / 3))
        return malloc(size);

    InitMemoryPool();

    // Align size to 16 bytes.
    size_t alignedSize = (size + 15) & ~static_cast<size_t>(15);

    // Try to reuse a free block.
    MemoryBlock* block = PopFreeBlock();
    while (block) {
        if (block->size >= alignedSize) {
            block->used = true;
            return block->address;
        }
        block = PopFreeBlock();
    }

    // Reserve new space from our pool.
    size_t currentOffset = gameMemoryPool.offset.fetch_add(alignedSize, std::memory_order_relaxed);
    if (currentOffset + alignedSize > TOTAL_POOL_SIZE)
        return nullptr; // Out of memory.

    void* allocatedMemory = gameMemoryPool.pool + currentOffset;

    // Create a new MemoryBlock to track this allocation.
    MemoryBlock* newBlock = new MemoryBlock{ alignedSize, true, allocatedMemory, nullptr };

    // Protect head insertion.
    {
        std::lock_guard<std::mutex> lock(headMutex);
        newBlock->next = gameMemoryPool.head.load(std::memory_order_acquire);
        gameMemoryPool.head.store(newBlock, std::memory_order_release);
    }

    return allocatedMemory;
}

// ---------------------------------------------------------------------------
// Custom Free (Lock-Free Free-List Insertion with Double-Free Check)
// ---------------------------------------------------------------------------
extern "C" __declspec(dllexport) void CustomFree(void* p) {
    if (!p || !gameMemoryPool.pool) return;  // Add pool check

    if (!IsInCustomPool(p)) {
        free(p);
        return;
    }

    MemoryBlock* targetBlock = nullptr;
    {
        std::lock_guard<std::mutex> lock(headMutex);
        MemoryBlock* current = gameMemoryPool.head.load(std::memory_order_acquire);
        while (current) {
            if (current->address == p) {
                if (!current->used) return;  // Already freed
                targetBlock = current;
                break;
            }
            current = current->next;
        }
    }

    if (targetBlock) {
        targetBlock->used = false;
        PushFreeBlock(targetBlock);
    }
}

// ---------------------------------------------------------------------------
// Custom Realloc (Minimizes Data Copying)
// ---------------------------------------------------------------------------
extern "C" __declspec(dllexport) void* CustomRealloc(void* p, size_t size) {
    if (!p) return CustomMalloc(size);
    if (!IsInCustomPool(p)) return realloc(p, size);

    MemoryBlock* origBlock = nullptr;
    {
        std::lock_guard<std::mutex> lock(headMutex);
        MemoryBlock* current = gameMemoryPool.head.load(std::memory_order_acquire);
        while (current) {
            if (current->address == p) {
                origBlock = current;
                break;
            }
            current = current->next;
        }
    }

    if (!origBlock) return realloc(p, size);

    void* newPtr = CustomMalloc(size);
    if (newPtr) {
        size_t copySize = (size < origBlock->size) ? size : origBlock->size;
        memcpy(newPtr, p, copySize);
        CustomFree(p);
    }
    return newPtr;
}

// ---------------------------------------------------------------------------
// Custom Calloc (Zero-initialized Memory)
// ---------------------------------------------------------------------------
extern "C" __declspec(dllexport) void* CustomCalloc(size_t n, size_t s) {
    size_t total = n * s;
    void* p = CustomMalloc(total);
    if (p) memset(p, 0, total);
    return p;
}

// ---------------------------------------------------------------------------
// Debugging Memory Usage (For Testing)
// ---------------------------------------------------------------------------
extern "C" __declspec(dllexport) void DebugMemoryUsage() {
    size_t usedMemory = 0, freeMemory = 0;
    {
        std::lock_guard<std::mutex> lock(headMutex);
        MemoryBlock* current = gameMemoryPool.head.load(std::memory_order_acquire);
        while (current) {
            if (current->used)
                usedMemory += current->size;
            else
                freeMemory += current->size;
            current = current->next;
        }
    }
}



// MEMORY ABOVE AS NOTE
// --------------------------------------------------
// Admin Check
// --------------------------------------------------
bool IsRunningAsAdmin() {
    BOOL isAdmin = FALSE;
    PSID adminGroup = nullptr;
    SID_IDENTIFIER_AUTHORITY ntAuthority = SECURITY_NT_AUTHORITY;
    if (AllocateAndInitializeSid(&ntAuthority, 2,
        SECURITY_BUILTIN_DOMAIN_RID, DOMAIN_ALIAS_RID_ADMINS,
        0, 0, 0, 0, 0, 0, &adminGroup))
    {
        CheckTokenMembership(nullptr, adminGroup, &isAdmin);
        FreeSid(adminGroup);
    }
    return (isAdmin != FALSE);
}

void RelaunchAsAdmin() {
    TCHAR szPath[MAX_PATH];
    if (GetModuleFileName(nullptr, szPath, MAX_PATH)) {
        SHELLEXECUTEINFO sei = { sizeof(sei) };
        sei.lpVerb = _T("runas");
        sei.lpFile = szPath;
        sei.hwnd = nullptr;
        sei.nShow = SW_NORMAL;
        if (!ShellExecuteEx(&sei)) {
            MessageBox(nullptr, _T("Must be run as administrator."), _T("Error"), MB_ICONERROR);
        }
        ExitProcess(0);
    }
}

// --------------------------------------------------
// Pick Soulstorm.exe
// --------------------------------------------------
static std::string PickSoulstormExe() {
    OPENFILENAMEA ofn = {};
    char fileName[MAX_PATH] = "";
    ofn.lStructSize = sizeof(ofn);
    ofn.lpstrFile = fileName;
    ofn.nMaxFile = MAX_PATH;
    ofn.lpstrFilter = "Exe Files\0*.exe\0All Files\0*.*\0";
    ofn.lpstrTitle = "Select Soulstorm.exe";
    ofn.Flags = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
    if (GetOpenFileNameA(&ofn)) {
        return std::string(fileName);
    }
    return {};
}

// --------------------------------------------------
// Enable LAA
// --------------------------------------------------
bool EnableLAA(const std::string& exePath) {
    DebugLogger::Log(DebugLogger::INFO, "EnableLAA: Opening file %s", exePath.c_str());
    std::fstream f(exePath.c_str(), std::ios::in | std::ios::out | std::ios::binary);
    if (!f.is_open()) {
        DebugLogger::Log(DebugLogger::CRITICAL, "EnableLAA: Failed to open file.");
        return false;
    }
    IMAGE_DOS_HEADER dosH = {};
    f.read(reinterpret_cast<char*>(&dosH), sizeof(dosH));
    if (dosH.e_magic != IMAGE_DOS_SIGNATURE) {
        DebugLogger::Log(DebugLogger::CRITICAL, "EnableLAA: Invalid DOS signature.");
        f.close();
        return false;
    }
    f.seekg(dosH.e_lfanew, std::ios::beg);
    IMAGE_NT_HEADERS32 nth = {};
    f.read(reinterpret_cast<char*>(&nth), sizeof(nth));
    if (nth.Signature != IMAGE_NT_SIGNATURE ||
        nth.OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR32_MAGIC)
    {
        DebugLogger::Log(DebugLogger::CRITICAL, "EnableLAA: Invalid NT header.");
        f.close();
        return false;
    }
    nth.FileHeader.Characteristics |= IMAGE_FILE_LARGE_ADDRESS_AWARE;
    f.clear();
    f.seekp(dosH.e_lfanew, std::ios::beg);
    f.write(reinterpret_cast<const char*>(&nth), sizeof(nth));
    f.close();
    DebugLogger::Log(DebugLogger::INFO, "EnableLAA: Succeeded.");
    return true;
}

// --------------------------------------------------
// Inject .injsec
// --------------------------------------------------
// --------------------------------------------------
// Inject .injsec Section into Executable
// --------------------------------------------------
bool InjectInjsec(const std::string& exePath) {
    DebugLogger::Log(DebugLogger::INFO, "InjectInjsec: Opening file %s", exePath.c_str());

    std::fstream f(exePath, std::ios::in | std::ios::out | std::ios::binary);
    if (!f.is_open()) {
        DebugLogger::Log(DebugLogger::CRITICAL, "InjectInjsec: Failed to open file.");
        return false;
    }

    // Read DOS header
    IMAGE_DOS_HEADER dosH = {};
    f.read(reinterpret_cast<char*>(&dosH), sizeof(dosH));
    if (dosH.e_magic != IMAGE_DOS_SIGNATURE) {
        DebugLogger::Log(DebugLogger::CRITICAL, "InjectInjsec: Invalid DOS signature.");
        f.close();
        return false;
    }

    // Read NT headers
    f.seekg(dosH.e_lfanew, std::ios::beg);
    IMAGE_NT_HEADERS32 nth = {};
    f.read(reinterpret_cast<char*>(&nth), sizeof(nth));
    if (nth.Signature != IMAGE_NT_SIGNATURE || nth.OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR32_MAGIC) {
        DebugLogger::Log(DebugLogger::CRITICAL, "InjectInjsec: Invalid NT header.");
        f.close();
        return false;
    }

    // Get section count
    WORD secCount = nth.FileHeader.NumberOfSections;
    DebugLogger::Log(DebugLogger::INFO, "InjectInjsec: Section count: %d", secCount);

    std::vector<IMAGE_SECTION_HEADER> secs(secCount);
    f.read(reinterpret_cast<char*>(secs.data()), secCount * sizeof(IMAGE_SECTION_HEADER));

    // Get alignment values
    DWORD fAlign = nth.OptionalHeader.FileAlignment;
    DWORD sAlign = nth.OptionalHeader.SectionAlignment;

    // Find last section
    auto AlignValue = [](DWORD value, DWORD alignment) {
        return (value + alignment - 1) & ~(alignment - 1);
        };
    IMAGE_SECTION_HEADER& lastSec = secs.back();
    DWORD newSecRaw = AlignValue(lastSec.PointerToRawData + lastSec.SizeOfRawData, fAlign);
    DWORD newSecVA = AlignValue(lastSec.VirtualAddress + lastSec.Misc.VirtualSize, sAlign);
    //check 1

    DebugLogger::Log(DebugLogger::INFO, "InjectInjsec: newSecRaw = %u, newSecVA = %u", newSecRaw, newSecVA);

    // Define the new section
    IMAGE_SECTION_HEADER inj = {};
    memcpy(inj.Name, ".injsec", 7);
    inj.VirtualAddress = newSecVA;
    inj.PointerToRawData = newSecRaw;
    inj.Misc.VirtualSize = 0x3000;  // 12 KB section size
    inj.SizeOfRawData = AlignValue(inj.Misc.VirtualSize, fAlign);
    inj.Characteristics = IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_EXECUTE | IMAGE_SCN_CNT_CODE;

    // Update headers
    nth.FileHeader.NumberOfSections++;
    nth.OptionalHeader.SizeOfImage = newSecVA + AlignValue(inj.Misc.VirtualSize, sAlign);

    // Write updates
    f.clear();
    f.seekp(dosH.e_lfanew, std::ios::beg);
    f.write(reinterpret_cast<const char*>(&nth), sizeof(nth));
    f.write(reinterpret_cast<const char*>(secs.data()), secCount * sizeof(IMAGE_SECTION_HEADER));
    f.write(reinterpret_cast<const char*>(&inj), sizeof(inj));

    // Zero out new section
    f.seekp(inj.PointerToRawData, std::ios::beg);
    std::vector<char> blank(inj.SizeOfRawData, 0);
    f.write(blank.data(), blank.size());

    f.close();

    DebugLogger::Log(DebugLogger::INFO, "InjectInjsec: Succeeded.");
    return true;
}

// Single function to apply entire patch steps
// Ensure RunPatch is declared before it's used



// ✅ Declare `RunPatch()` BEFORE its first use
void RunPatch() {
    DebugLogger::Log(DebugLogger::INFO, "RunPatch: Executing in-game modifications...");
    PatchMultiplayerLobby(); // Applies the multiplayer patch
}

// chatgpt saynid void patch here? STUPID

// ✅ `PatchSoulstorm()` Now Finds `RunPatch()`
bool PatchSoulstorm(const std::string& exePath) {
    DebugLogger::Log(DebugLogger::INFO, "PatchSoulstorm: Starting patch for %s", exePath.c_str());

    // 1) Enable Large Address Aware (LAA)
    if (!EnableLAA(exePath)) {
        DebugLogger::Log(DebugLogger::CRITICAL, "PatchSoulstorm: EnableLAA failed.");
        return false;
    }
    DebugLogger::Log(DebugLogger::INFO, "[OK] EnableLAA applied successfully.");

    // 2) Inject .injsec section
    if (!InjectInjsec(exePath)) {
        DebugLogger::Log(DebugLogger::CRITICAL, "PatchSoulstorm: InjectInjsec failed.");
        return false;
    }
    DebugLogger::Log(DebugLogger::INFO, "[OK] InjectInjsec applied successfully.");

    // 3) Apply runtime memory modifications
    DebugLogger::Log(DebugLogger::INFO, "[->] Executing RunPatch() for runtime modifications...");

    RunPatch(); // RunPatch should now be defined before use

    DebugLogger::Log(DebugLogger::INFO, "[OK] RunPatch() executed successfully.");
    DebugLogger::Log(DebugLogger::INFO, "PatchSoulstorm: All patch steps completed.");

    return true;
}

// ✅ Main function (WinMain)
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    DebugLogger::Init();
    DebugLogger::Log(DebugLogger::INFO, "WinMain started.");

    // Ensure admin rights
    if (!IsRunningAsAdmin()) {
        DebugLogger::Log(DebugLogger::CRITICAL, "Not running as admin. Relaunching...");
        RelaunchAsAdmin();
        return 0;
    }

    // Prompt user for Soulstorm.exe
    std::string soulExe = PickSoulstormExe();
    if (soulExe.empty()) {
        DebugLogger::Log(DebugLogger::CRITICAL, "No EXE selected by user.");
        MessageBoxA(nullptr, "No EXE selected.", "Error", MB_ICONERROR);
        return 1;
    }

    DebugLogger::Log(DebugLogger::INFO, "User selected EXE: %s", soulExe.c_str());

    // Load GDI Hook DLL and check hook status
    try {
        DebugLogger::Log(DebugLogger::INFO, "Loading GDI Hooking DLL...");
        const std::wstring gdiHookPath = L"C:\\Program Files (x86)\\Steam\\steamapps\\common\\Dawn of War Soulstorm\\GDI HOOKING DLL.dll";

        DllHandle gdiHookDll = loadDll(gdiHookPath);
        DebugLogger::Log(DebugLogger::INFO, "GDI Hooking DLL loaded successfully.");

        // Apply patch
        if (!PatchSoulstorm(soulExe)) {
            DebugLogger::Log(DebugLogger::CRITICAL, "PatchSoulstorm failed.");
            MessageBoxA(nullptr, "Patch failed – multiplayer net-lobby might remain hidden.", "Patch Error", MB_ICONERROR);
            return 1;
        }

        // Success message
        MessageBoxA(nullptr,
            "Soulstorm Patch Applied:\n\n"
            " - LAA enabled\n"
            " - .injsec added\n"
            " - Multiplayer lobby JNZ patched\n"
            " - Custom memory functions in .injsec\n\n"
            "Success!",
            "Done", MB_OK);
        DebugLogger::Log(DebugLogger::INFO, "All done.");
    }
    catch (const std::exception& e) {
        DebugLogger::Log(DebugLogger::CRITICAL, "Exception caught: %s", e.what());
        MessageBoxA(nullptr, e.what(), "Error", MB_ICONERROR);
        return 1;
    }
    catch (...) {
        DebugLogger::Log(DebugLogger::CRITICAL, "Unknown exception caught.");
        MessageBoxA(nullptr, "Unknown error occurred.", "Error", MB_ICONERROR);
        return 1;
    }
}